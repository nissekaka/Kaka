...
float3 EvaluateAmbianceDynamicSky(SamplerState samplerState, TextureCube lysBurleyCube0, TextureCube lysBurleyCube1, float blendFactor, float3 vN, float3 VNUnit, float3 toEye, float perceptualRoughness, float ao, float3 dfcol, float3 spccol)
{
    int numMips = GetNumMips(lysBurleyCube0);
    const int nrBrdMips = numMips - nMipOffset;
    float VdotN = saturate(dot(toEye, vN)); //clamp(dot(toEye, vN), 0.0, 1.0f);
    const float3 vRorg = 2 * vN * VdotN - toEye;
    ...
    float3 specRad0 = lysBurleyCube0.SampleLevel(samplerState, vR, mipLevel).xyz;
    float3 diffRad0 = lysBurleyCube0.SampleLevel(samplerState, vN, (float) (nrBrdMips - 1)).xyz;
    float3 specRad1 = lysBurleyCube1.SampleLevel(samplerState, vR, mipLevel).xyz;
    float3 diffRad1 = lysBurleyCube1.SampleLevel(samplerState, vN, (float) (nrBrdMips - 1)).xyz;

    const float4 specRad = float4(lerp(specRad0, specRad1, blendFactor), 1.0f);
    const float4 diffRad = float4(lerp(diffRad0, diffRad1, blendFactor), 1.0f);
